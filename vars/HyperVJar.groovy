 ...
 // 	def APP_HOSTNAMES=config.APP_HOSTNAMES.split(',');
  ...
  def MID=APP_HOSTSIZE/2
	...
	for (i = 0; i<APP_LEFT_HOSTS.size; i++) {
		...
		//sh  "ssh ${saltmasterIP}  'sudo salt -S \"${APP_LEFT_HOST}\" cmd.script salt://scripts/update_jar.sh \"update-all ${TOMCAT_HOME} ${dstPackageName} ${dir_update} ${CheckUrl} ${APP_LEFT_HOST} ${APP_PORT}\" runas=\"${AppRunAs}\" ' ";
		sh (script: "ssh ${saltmasterIP}  'sudo salt -S \"${APP_LEFT_HOST}\" cmd.script salt://scripts/update_jar.sh \"update-all ${TOMCAT_HOME} ${dstPackageName} ${dir_update} ${CheckUrl} ${APP_LEFT_HOST} ${APP_PORT}\" runas=\"${AppRunAs}\" ' ",returnStdout: true);
	}
	...
	for (i = 0; i<APP_RIGHT_HOSTS.size; i++) {
		...
		//sh  "ssh ${saltmasterIP}  'sudo salt -S \"${APP_RIGHT_HOST}\" cmd.script salt://scripts/update_jar.sh \"update-all ${TOMCAT_HOME} ${dstPackageName} ${dir_update} ${CheckUrl} ${APP_RIGHT_HOST} ${APP_PORT}\" runas=\"${AppRunAs}\" ' ";
		sh (script: "ssh ${saltmasterIP}  'sudo salt -S \"${APP_RIGHT_HOST}\" cmd.script salt://scripts/update_jar.sh \"update-all ${TOMCAT_HOME} ${dstPackageName} ${dir_update} ${CheckUrl} ${APP_RIGHT_HOST} ${APP_PORT}\" runas=\"${AppRunAs}\" ' ",returnStdout: true);
	}
