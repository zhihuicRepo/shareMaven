...
def config = [:]
 ...
 // 	def APP_HOSTNAMES=config.APP_HOSTNAMES.split(',');
  ...
  def MID=APP_HOSTSIZE/2
	...
	for (i = 0; i<APP_LEFT_HOSTS.size; i++) {
		def APP_LEFT_HOST=APP_LEFT_HOSTS[i].trim();
		sh  "ssh ${saltmasterIP}  'sudo salt -S \"${APP_LEFT_HOST}\" cmd.script salt://scripts/update_jar.sh \"check_url ${TOMCAT_HOME} ${dstPackageName} ${dir_update} ${CheckUrl} ${APP_LEFT_HOST} ${APP_PORT}\" runas=\"${AppRunAs}\" ' ";
	}
	// nginx reload
	for (i = 0; i<APP_LEFT_HOSTS.size; i++) {
		def APP_LEFT_HOST=APP_LEFT_HOSTS[i].trim();
	...
	for (i = 0; i<APP_RIGHT_HOSTS.size; i++) {
		def APP_RIGHT_HOST=APP_RIGHT_HOSTS[i].trim();
		sh  "ssh ${saltmasterIP}  'sudo salt -S \"${APP_RIGHT_HOST}\" cmd.script salt://scripts/update_jar.sh \"check_url ${TOMCAT_HOME} ${dstPackageName} ${dir_update} ${CheckUrl} ${APP_RIGHT_HOST} ${APP_PORT}\" runas=\"${AppRunAs}\" ' ";
	}
	// nginx reload
	for (i = 0; i<APP_RIGHT_HOSTS.size; i++) {
		def APP_RIGHT_HOST=APP_RIGHT_HOSTS[i].trim();
